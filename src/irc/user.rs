/// User management: local nick tracking, nick validation, and caching of
/// WHOIS / WHO results received from the server.
...
/// Tracks the local user's nickname, pending nick changes, and cached
/// WHOIS / WHO results from the server.
...
impl UserManager {
    ...
    pub fn request_nick_change(&mut self, new_nick: &str) -> Result<OutboundMessage> {
        ...
        Ok(OutboundMessage::Nick { new_nick: new_nick.to_string() })
    }
    ...
    pub fn build_whois(&self, nick: &str) -> Result<OutboundMessage> {
        ...
        Ok(OutboundMessage::Whois { nick: nick.to_string() })
    }
    ...
    pub fn handle_whois_user(&mut self, nick: &str, username: &str, host: &str, realname: &str) {
        let entry = self
            ...
            .or_insert_with(|| WhoisInfo { nick: nick.to_string(), ..Default::default() });
    ...
    pub fn build_who(&self, mask: &str) -> Result<OutboundMessage> {
        ...
        Ok(OutboundMessage::Who { mask: mask.to_string() })
    }
    ...
    pub fn handle_who_reply(
        ...
        self.who_results.retain(|e| !e.nick.eq_ignore_ascii_case(nick));
